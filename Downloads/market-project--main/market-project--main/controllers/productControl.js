...
const createProduct = asyncHandler(async (req, res) => {
  ...
  for (let oneSize of size) {
    const newSize = new Size({ name: oneSize.name, productId: pro._id,quantity:oneSize.quantity });
    await newSize.save();
  }
  // creat color
...
const getProduct = asyncHandler(async (req, res) => {
    ...
    const listSize=await Size.find({productId:req.params.id}).select("-productId -_id -__v");
    const listColor=await Color.find({productId:req.params.id}).select("-productId -_id -__v");
        const colorNames = listColor.map(c => c.name);

    res.status(200).json({Pro,categoryNames,listSize,colorNames});
